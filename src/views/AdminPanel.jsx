...
import {
  isDeactivated, listenAllUsers, roleLabel, ROLES, setUserDeactivated, setUserRole,
} from '../lib/users'
...
export default function AdminPanel() {
  ...
  // Removed accounts sink to the bottom rather than being hidden: this is the
  // one screen that has to show them, because it is the only way back.
  const visible = useMemo(() => {
    const f = filter.trim().toLowerCase()
    return users
      .filter(u => !f || u.name?.toLowerCase().includes(f) || u.email?.toLowerCase().includes(f))
      .sort((a, b) => (isDeactivated(a) ? 1 : 0) - (isDeactivated(b) ? 1 : 0))
  }, [users, filter])
  ...
  // into NaN. Removed accounts are counted separately rather than inflating
  // the tier they used to hold.
  const counts = useMemo(() => {
    const c = { developer: 0, super_admin: 0, admin: 0, employee: 0, guest: 0, removed: 0 }
    for (const u of users) {
      if (isDeactivated(u)) { c.removed++; continue }
  ...
  const onToggleDeactivated = async (u) => {
    setError(null)
    const off = !isDeactivated(u)
    // Removing yourself would sign you out on the next reload with no way to
    // undo it from inside the app.
    if (off && u.id === profile.id) {
      setError("You can't remove your own account. Ask another super admin or developer to do it.")
      return
    }
    // Mirrors developerRoleChangeAllowed(), which treats any write to a
    // developer's document the same as a demotion — otherwise removing them
    // would bypass the boundary exactly as demoting them would.
    if (isDeveloper(u) && !mayGrantDev) {
      setError('Only a developer can remove or restore a developer account.')
      return
    }
    if (off && !confirm(
      `Remove ${u.name || u.email} from the workspace?\n\n`
      + 'They keep their Google account but the app will refuse them everything, '
      + 'and they drop out of DM and member lists. Their messages stay. '
      + 'You can restore them here later.'
    )) return
    setBusyUid(u.id)
    try { await setUserDeactivated(u.id, off, profile.id) }
      ...
      <div className="flex-1 overflow-y-auto scrollbar-thin p-6">
        <div className="max-w-3xl mx-auto">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-6 gap-3 mb-6">
            ...
            <Stat label="Removed"      value={counts.removed}     accent="text-ink-dim" />
          </div>
          ...
          <div className="bg-bg-raised border border-line-subtle rounded-lg overflow-hidden">
            <div className="grid grid-cols-[1fr_auto_auto_auto] gap-3 px-4 py-2 text-[11px] uppercase tracking-wider text-ink-dim border-b border-line-subtle bg-bg-deepest font-semibold">
              <div>Member</div>
            ...
            {visible.map(u => {
              const removed = isDeactivated(u)
              const devLocked = isDeveloper(u) && !mayGrantDev
              return (
              <div key={u.id} className="grid grid-cols-[1fr_auto_auto_auto] gap-3 px-4 py-3 items-center border-b border-line-subtle last:border-b-0 hover:bg-bg-hover/50">
                <div className={`flex items-center gap-3 min-w-0 ${removed ? 'opacity-50' : ''}`}>
                  <Avatar name={u.name} src={u.photoURL} size={32} />
                  <div className="min-w-0">
                    <div className="text-sm font-medium truncate flex items-center gap-2">
                      <span className={`truncate ${removed ? 'line-through' : ''}`}>{u.name}</span>
                      {u.id === profile.id && (
                        <span className="text-[10px] text-ink-dim">(you)</span>
                      )}
                      {removed && (
                        <span className="text-[10px] uppercase tracking-wider text-ink-dim border border-line-subtle rounded px-1 shrink-0">
                          Removed
                        </span>
                      )}
                    </div>
                ...
                <div className="text-right">
                  <select
                    disabled={busyUid === u.id || devLocked || removed}
                    title={devLocked
                      ? 'Only a developer can change a developer\'s role'
                      : removed ? 'Restore this account before changing its role' : undefined}
                ...
                <div className="text-right">
                  <button
                    onClick={() => onToggleDeactivated(u)}
                    disabled={busyUid === u.id || devLocked || u.id === profile.id}
                    title={u.id === profile.id
                      ? "You can't remove your own account"
                      : devLocked ? 'Only a developer can remove a developer' : undefined}
                    className={`px-2 py-1 text-xs rounded-md border disabled:opacity-40 disabled:cursor-not-allowed ${
                      removed
                        ? 'border-line-subtle text-ink-muted hover:text-ink hover:bg-bg-hover'
          ...
          <p className="text-xs text-ink-dim mt-6">
            <strong className="text-ink-muted">Removing someone</strong> refuses their account
            everything and drops them out of DM lists, member lists and mention pickers, while
            leaving their messages and their group memberships intact &mdash; so restoring them
            gives back exactly what they had. It does not delete their Google account, which
            this app has no way to do; the workspace simply stops answering to it. Retired bots
            leave the DM list the same way, by being switched off under Bots below.
          </p>
